...
Para usar el ejercicio, ingrese al siguiente enlace: [Otros](https://velkez.github.io/naddie-prueba-tecnica-2/editor/)
...
- **Activación desde Menú:** Al hacer clic en `Add > Other > Evento`, se injectó la propiedad inicial `userData.activeEvent = 'Ninguno'` al objeto seleccionado y se disparó la señal `signals.objectChanged.dispatch( object )` para forzar la actualización de la UI.
...
Al seleccionar `Add > Other > Imagen`, se habilita el flujo de carga:
...
flowchart TD
    ...
    Start((Inicio)) --> TopMenu[Menubar: Add > Other > Imagen]
