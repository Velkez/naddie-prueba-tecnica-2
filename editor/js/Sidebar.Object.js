...
import { UIPanel, UIRow, UIInput, UIButton, UIColor, UICheckbox, UIInteger, UIText, UINumber } from './libs/ui.js';
...
function SidebarObject( editor ) {
	...
	imagePanel.setMarginTop( '10px' );

	const imageOpacityRow = new UIRow();
	const imageOpacityLabel = new UIText( strings.getKey( 'sidebar/object/opacity' ) ).setClass( 'Label' );
	const imageOpacity = new UINumber( 1 ).setRange( 0, 1 ).setWidth( '50px' ).onChange( update );

	imageOpacityRow.add( imageOpacityLabel );
	imageOpacityRow.add( imageOpacity );

	imagePanel.add( imageOpacityRow );
	...
	function update() {
		...
		if ( object !== null ) {
			...
			if ( object.userData && object.userData.type === 'ui-image' ) {

				if ( object.material && object.material.opacity !== imageOpacity.getValue() ) {

					editor.execute( new SetValueCommand( editor, object.material, 'opacity', imageOpacity.getValue() ) );
	...
	function updateUI( object ) {
		...
		objectType.setValue( object.userData && object.userData.type === 'ui-image' ? 'Imagen' : object.type );
		...
		if ( object.userData && object.userData.type === 'ui-image' ) {
			...
			imagePanel.setDisplay( 'block' );
			if ( object.material ) {

				imageOpacity.setValue( object.material.opacity );

			}

		} else {

			imagePanel.setDisplay( 'none' );
